...
!***END PROLOGUE  DCSEVL
    ...
    integer(4) :: N
    integer(4) :: I, NI
  real(8) :: B0, B1, B2, ONEPL, TWOX
  LOGICAL FIRST
  SAVE FIRST, ONEPL
  DATA FIRST /.TRUE./
!***FIRST EXECUTABLE STATEMENT  DCSEVL
  if (FIRST) ONEPL = 1.0D0 + 2.0d0*spacing(1.0d0)
  FIRST = .FALSE.
  if (N  <  1) stop 'SLATEC DCSEVL NUMBER OF TERMS  <=  0'
  if (N  >  1000) stop 'SLATEC DCSEVL NUMBER OF TERMS  >  1000'
  if (ABS(X)  >  ONEPL) stop 'SLATEC DCSEVL X OUTSIDE THE INTERVAL (-1,+1)'
!
  B1 = 0.0D0
  B0 = 0.0D0
  TWOX = 2.0D0*X
...
!***END PROLOGUE  DE1
  integer(4) :: NTAE10, NTAE11, NTAE12, NTE11, NTE12, &
       NTAE13, NTAE14

  real(8) :: ETA
  real(8) :: DE1
  real(8) :: X, AE10CS(50), AE11CS(60), AE12CS(41), E11CS(29), &
    E12CS(25), AE13CS(50), AE14CS(64), XMAX, XMAXT, D1MACH
  LOGICAL FIRST
  SAVE AE10CS, AE11CS, AE12CS, E11CS, E12CS, AE13CS, AE14CS, &
   NTAE10, NTAE11, NTAE12, NTE11, NTE12, NTAE13, NTAE14, XMAX, &
   FIRST
...
!***FIRST EXECUTABLE STATEMENT  INITDS
  if (NOS  <  1) stop 'SLATEC INITDS Number of coefficients is less than 1'
!
  ERR = 0.
  DO 10 II = 1,NOS
    I = NOS + 1 - II
   ...
   10 CONTINUE
!
   20 if (I  ==  NOS) stop 'SLATEC INITDS Chebyshev series too short for specified accuracy'
  INITDS = I
!
  return
end function INITDS
