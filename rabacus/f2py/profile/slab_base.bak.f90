...
contains
  ...
  subroutine set_recomb_photons( &
    ...
    ! calculate angle averaged specific intensities
    !------------------------------------------------
    ...
    !$omp  do private(k, i, dtau_H1_th_k, dtau_He1_th_k, dtau_He2_th_k, &
    !$omp&  sigma_H1, sigma_H1_ra, sigma_He1, sigma_He1_ra, sigma_He2, &
    !$omp&  sigma_He2_ra, tau )
    
    over_solve_layers: do k = 0, Nl-1

       ! for each solve layer with central position z_c we do two integrals
       ! one from 0 - z_c and one from z_c - L. 

       dtau_H1_th_k = dtau_H1_th(k)
       dtau_He1_th_k = dtau_He1_th(k)
       dtau_He2_th_k = dtau_He2_th(k)
       ...
       tau = dtau_H1_th_k * sigma_H1_ra * half
       do i = k-1, 0, -1 
          tau = tau + dtau_H1_th(i) * sigma_H1_ra
       ...
       tau = (dtau_H1_th_k * sigma_H1_ra + &
              dtau_He1_th_k * sigma_He1_ra) * half
       do i = k-1, 0, -1
          tau = tau + dtau_H1_th(i) * sigma_H1_ra + &
                      dtau_He1_th(i) * sigma_He1_ra
       ...
       tau = ( dtau_H1_th(i) * sigma_H1_ra + &
               dtau_He1_th(i) * sigma_He1_ra ) * half
       do i = k+1, Nl-1
          tau = tau + dtau_H1_th(i) * sigma_H1_ra + &
                      dtau_He1_th(i) * sigma_He1_ra
          J_He2(k) = J_He2(k) + dl(i) * em_He2(i) * e1xa( tau ) 
       ...
       tau = (dtau_H1_th_k * sigma_H1_ra + &
              dtau_He1_th_k * sigma_He1_ra + &
              dtau_He2_th_k * sigma_He2_ra ) * half
       do i = k-1, 0, -1
          tau = tau + dtau_H1_th(i) * sigma_H1_ra + &
                      dtau_He1_th(i) * sigma_He1_ra + &
                      dtau_He2_th(i) * sigma_He2_ra
